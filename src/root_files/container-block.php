...
class Cobl_Container_block {
	...
	public function on_activate() {
		if ( $this->check_dependencies() ){
			...
			do_action('cobl_on_activate_before_flush');
			flush_rewrite_rules();
			$this->maybe_update();
			do_action('cobl_plugin_activated');
		} else {
			add_action( 'admin_init', array( $this, 'deactivate' ) );
			wp_die(
	...
	public function on_deactivate() {
		$this->add_roles_and_capabilities();
		do_action('cobl_on_deactivate_before_flush');
		flush_rewrite_rules();
		do_action('cobl_plugin_deactivated');
	}

	public static function on_uninstall() {
		do_action('cobl_plugin_uninstalled');
	}
	...
	protected function init_options() {
		update_option( 'cobl_version', self::VERSION );
		add_option( 'cobl_db_version', self::DB_VERSION );
	}
	...
	protected function maybe_update() {
		if ( get_option( 'cobl_db_version' ) < self::DB_VERSION ) {
			// require_once( self::plugin_dir_path() . 'inc/dep/class-cobl-update.php' );
			// new Cobl_Update();
			// class Cobl_Update()  is missing ??? !!!
		}
	}
...
function cobl_container_block_init() {
	return Cobl_Container_block::get_instance();
}
cobl_container_block_init();
